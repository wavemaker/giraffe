  ...
  return graph = new graph_provider({
    ...
    onComplete: function(transport) {
      ...
      if (metric.annotator || metric.events) {
        ...
        onError: function(){
            $(".loadingImg").remove();
            $(renderAt).after("<span class='graph-no-data'>Unable to fetch data</span>")
        }
    ...
    return deferred = $.ajax({
      ...
      url: generateDataURL(this.args.targets, this.args.annotator_target, this.args.width),
            success:function(data){
                $(".loadingImg").remove();
                if(!data.length){
                    $(renderAt).after("<span class='graph-no-data'>No Data Available</span>");
                }
            },
...
$(document).on('click', 'a.range', function() {
	  var timeFrame, _ref;
	  period = Number($(this).attr('data-timeframe')) || default_period;
	  dataPoll();
